	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		switch($dest) {
			case 'I': {
				...
				if (ob_get_contents()) {
					$this->Error(' Fala sério cara!!!!!! Some data has already been output, can\'t send PDF file');
				}
				...
				if (php_sapi_name() != 'cli') {
					...
					if (headers_sent()) {
						$this->Error(' caso I fiote de cruz credo Some data has already been output to browser, can\'t send PDF file');
					}
			...
			case 'D': {
				...
				if (ob_get_contents()) {
					$this->Error(' CASO D MAN =D Some data has already been output, can\'t send PDF file');
				}
				header('Content-Description: File Transfer');
				if (headers_sent()) {
					$this->Error(' caso d tio Some data has already been output to browser, can\'t send PDF file');
				}
				header('Cache-Control: private, must-revalidate, post-check=0, pre-check=0, max-age=1');
				//header('Cache-Control: public, must-revalidate, max-age=0'); // HTTP/1.1
				header('Pragma: public');
